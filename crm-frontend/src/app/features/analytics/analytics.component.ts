      ...
      <div *ngIf="!loading">
        ...
        <div class="summary-row">
          <div class="sum-card">
            ...
            <div class="sum-label">Total Conversations</div>
          </div>
          <div class="sum-card">
...
[attr.stroke-dashoffset]="71 - openArc"
        ...
        <div class="chart-card full">
          <div class="chart-title">Top Messages</div>
          ...
          <div class="senders-grid">
            <div *ngFor="let s of senderData; let i = index" class="sender-row">
              ...
              <span class="sender-count">{{ s.count }} messages</span>            
            </div>
          </div>
        </div>
      </div>
...
({
  ...
  get senderData() {
  const max = Math.max(...this.tickets.map(t => t.messageCount || 1), 1);
  return this.tickets
    .filter(t => (t.messageCount || 0) > 0)
    .map(t => ({
      senderId: t.senderName || t.senderId,
      count: t.messageCount || 1,
      pct: ((t.messageCount || 1) / max) * 100
    }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 8);
}
