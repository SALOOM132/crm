...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        <div>
          ...
          <p class="page-sub">Super Admin — full control</p>
        </div>
      ...
      <div *ngIf="!loading" class="users-table">
        ...
        <div *ngFor="let user of users" class="table-row">
          ...
          <span class="actions">
            <button class="btn-action" (click)="toggle(user)"
      ...
      <div class="modal" (click)="stopProp($event)">
        ...
        <div class="modal-body">
          ...
          <div class="field">
            ...
            <input type="text" [(ngModel)]="newAdmin.fullName" placeholder="e.g. John Admin"/>
          </div>
          <div class="field">
            <label>Username</label>
            <input type="text" [(ngModel)]="newAdmin.username" placeholder="e.g. johnadmin"/>
          </div>
          <div class="field">
            <label>Password</label>
            <input type="password" [(ngModel)]="newAdmin.password" placeholder="Min 6 characters"/>
          </div>
        </div>
  ...
  styles: [`
    ...
    .section-title { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 600; color: #9090b0; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 12px; }
    ...
    .table-row { display: grid; grid-template-columns: 1.5fr 1fr 1fr 0.8fr 100px; align-items: center; padding: 14px 20px; border-bottom: 1px solid rgba(255,255,255,0.04); font-size: 13px; gap: 12px; }
    ...
    .role-badge { display: inline-block; padding: 3px 8px; border-radius: 4px; font-size: 10px; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase; }
    .role-super_admin { background: rgba(216,90,48,0.15); color: #f0997b; }
    .role-admin { background: rgba(99,102,241,0.15); color: #a5b4fc; }
    .role-agent { background: rgba(29,158,117,0.15); color: #34d399; }
    ...
    .btn-action:hover:not(:disabled) { color: #e8e8f0; }
    .btn-action.delete:hover:not(:disabled) { color: #f87171; background: rgba(239,68,68,0.1); }
    ...
    .field input { width: 100%; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; color: #e8e8f0; padding: 9px 12px; font-family: 'DM Sans', sans-serif; font-size: 13px; outline: none; box-sizing: border-box; transition: border-color 0.2s; }
...
export class SuperAdminComponent implements OnInit {
  ...
  creating = false;
  createError = '';
  newAdmin = { fullName: '', username: '', password: '' };
  ...
  createAdmin() {
    this.createError = '';
    if (!this.newAdmin.fullName || !this.newAdmin.username || !this.newAdmin.password) {
    ...
    this.svc.createAdmin(this.newAdmin).subscribe({
      next: () => {
        ...
        this.newAdmin = { fullName: '', username: '', password: '' };
        this.load();
      },
      error: (err) => {
        this.creating = false;
        this.createError = err.error?.error || 'Failed to create admin';
      }
    });
  }

  toggle(user: AppUser) {
    this.svc.toggleUser(user.id).subscribe({
      next: (updated) => {
  ...
  deleteUser(user: AppUser) {
    if (!confirm(`Delete admin ${user.fullName}?`)) return;
    this.svc.deleteAdmin(user.id).subscribe({
