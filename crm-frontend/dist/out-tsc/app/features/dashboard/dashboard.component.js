...
let DashboardComponent = class DashboardComponent {
    ...
    createTestTicket() {
        if (!this.testMessage)
            return;
        this.creating = true;
        this.ticketService.createTicket({ content: this.testMessage }).subscribe({
            next: () => {
                this.creating = false;
                this.showTestModal = false;
                this.testMessage = '';
...
DashboardComponent = __decorate([
    Component({
        ...
        standalone: true,
        imports: [CommonModule, NgIf, NgFor, RouterLink, FormsModule, SlicePipe], template: `
    <div class="page">
      <!-- Header -->
      ...
      <div class="section">
        ...
        <div *ngIf="!loading && tickets.length === 0" class="empty-state">
          ...
          <p>No tickets yet. Messages from Instagram & Messenger will appear here.</p>
        </div>
        ...
        <div *ngIf="!loading" class="tickets-table">
          ...
          <div *ngFor="let ticket of recentTickets" class="table-row" [routerLink]="['/tickets', ticket.id]">
<span class="msg-cell">{{ (ticket.message || '') | slice:0:60 }}{{ (ticket.message?.length ?? 0) > 60 ? '...' : '' }}</span>            <span class="sender-cell">
